...
class GEOMDataSet(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, pickle_path in tqdm(enumerate(pickle_paths)):
            try:
                with open(pickle_path, "rb") as f:
                    mol_dict = pickle.load(f)

                molecule_data_list = self.process_molecule(mol_dict)
                if molecule_data_list is not None:
                    data_list.extend(molecule_data_list)

                if i == 10000:
                    logging.info("Loaded 100 molecule pickles...")
                    break
            except Exception as e:
    ...
    def process_molecule(self, mol_dict):
        try:
            ...
            energies = np.array(
                [conformer["totalenergy"] for conformer in conformers_info], dtype=float
            )
            ...
            keep = order[: self.num_conformers]
